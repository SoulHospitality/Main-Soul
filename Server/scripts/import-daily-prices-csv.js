...
const CSV_PATH =
  process.argv[2] || path.join(process.env.USERPROFILE || '', 'Downloads', 'unit_daily_prices_rows (1).csv');
const BATCH = 500;
...
async function flushBatch(client, batch) {
  ...
  for (const row of batch) {
    values.push(`($${p++},$${p++},$${p++},$${p++},$${p++},COALESCE($${p++}::timestamptz, now()))`);
    params.push(row.wp_post_id, row.date, row.price, row.currency, row.source, row.updated_at);
  }
...
async function main() {
  ...
  try {
    ...
    for (let idx = 0; idx < records.length; idx++) {
      ...
      if (!wpPostId || !date || !price || price <= 0) {
        skipped += 1;
        if (errors.length < 20) errors.push(`row ${idx + 2}: invalid wp_post_id/date/price`);
        continue;
      }

      const currency = emptyToNull(row.currency) === 'USD' ? 'USD' : 'EGP';
      ...
      if (batch.length >= BATCH) {
        ...
        batch = [];
        if (upserted % 2000 === 0) console.log(`… ${upserted} upserted`);
      }
    }
  ...
  const { rows: countRows } = await pool.query(`SELECT COUNT(*)::int AS n FROM unit_daily_prices`);
  const { rows: unitRows } = await pool.query(
    `SELECT COUNT(DISTINCT wp_post_id)::int AS units FROM unit_daily_prices`
  );
    ...
    JSON.stringify(
      {
        upserted,
        skipped,
        totalPrices: countRows[0].n,
        unitsWithPrices: unitRows[0].units,
        errors,
      },
      null,
      2
