/**
 * Wipe all PMS reservations, then import North Coast sheet exactly.
 * Red-highlighted Excel rows OR blank/unmatched unit → status=cancelled (kept in history).
 *
 * Usage:
 *   node scripts/import_north_coast_reservations.js --dry-run
 *   node scripts/import_north_coast_reservations.js
 */
...
const FILE =
  process.env.NC_RESERVATIONS_XLSX ||
  'C:/Users/hazem/Downloads/All north coast reservations.xlsx';
const DRY = process.argv.includes('--dry-run');
const EXPECTED = 538;
const SOURCE_TAG = '[xlsx-import:north-coast-clean]';
...
function codeVariants(raw) {
  ...
  if (!base) return set;
  if (base.startsWith('GAIA-')) {
    set.add(base.slice(5));
    set.add(compact(base.slice(5)));
  }
  if (compact(raw) === 'GAIAZ106' || compact(raw) === 'Z106') {
    set.add('GAIAZ106');
    set.add('Z106');
  }
  ...
  // CL11-CH8A-02 ↔ CL11-CH8-02 style
  set.add(base.replace(/([A-Z])(\d{2})$/, '-$1$2').replace(/--/g, '-'));
  return set;
}
...
function parseWorkbook(file) {
  ...
  const col = (name, fallback) => (header[name] != null ? header[name] : fallback);
  const cIn = col('Check In', 0);
  const cOut = col('Check out', 1);
  const cUnit = col('Unit No.', 2);
  const cName = col('Client Name', 3);
  const cMobile = col('Mobile No.', 4);
  const cGuests = col('Number Of Guests', 5);
  const cPay = col('Payment Method', 9);
  const cNights = col('Nights ', 10);
  const cPpn = col('Price per Night', 11);
  const cTotal = col('Total Reservation', 14);
  const cDown = col('Down Payment', 15);
  const cIns = col('Insurance ', 17);
  const cHk = col('Housekeeping', 18);
  const cBeach = col('Beach Pass', 19);
  const cUtil = col('Total Utilites', 13);
  const cSource = col('Source', 21);
  const cSales = col('Sales Name', 22);
  const cNotes = col('Notes', 32);

  const rows = [];
  for (let R = 1; R <= range.e.r; R++) {
    const get = (c) => sheet[XLSX.utils.encode_cell({ r: R, c })];
    ...
    rows.push({
      ...
      salesName,
      notes: str(get(cNotes) && get(cNotes).v),
...
async function wipeAll() {
  await query(`UPDATE petty_cash SET linked_reservation_id = NULL WHERE linked_reservation_id IS NOT NULL`);
  await query(`DELETE FROM commissions WHERE reservation_id IS NOT NULL`);
  await query(`DELETE FROM payments WHERE reservation_id IS NOT NULL`);
  await query(`DELETE FROM housekeeping_tasks WHERE reservation_id IS NOT NULL`);
  // best-effort extras
  await query(`DELETE FROM whatsapp_outbound_log WHERE reservation_id IS NOT NULL`).catch(() => {});
  const { rows } = await query(`DELETE FROM reservations RETURNING id`);
  await query(
    `DELETE FROM unit_blocked_dates WHERE source IN ('reservation_import','csv_import','reservation')`
  ).catch(() => {});
  return rows.length;
}
...
async function main() {
  console.log(`[nc-import] file=${FILE} dry=${DRY}`);
  const parsed = parseWorkbook(FILE);
  console.log(
    `[nc-import] parsed=${parsed.length} cancelled=${parsed.filter((r) => r.cancelled).length}`
  );
  ...
  if (badDates.length) {
    console.log('[warn] bad dates', badDates.slice(0, 20).map((r) => r.excelRow));
  ...
  for (const r of parsed) {
    ...
    r.unit = unit;
    // Blank / unmatched unit codes are cancelled (still imported).
  ...
  console.log(`[nc-import] matchedUnits=${matched} unmatchedCodes=${unmatched.size}`);
  for (const [k, n] of [...unmatched.entries()].sort((a, b) => b[1] - a[1])) {
    console.log(`  - ${k}: ${n}`);
  }

  if (DRY) {
    console.log('[nc-import] dry-run complete — no writes');
    await pool.end();
    return;
  }

  const wiped = await wipeAll();
  console.log(`[nc-import] wiped reservations=${wiped}`);
  ...
  for (const r of parsed) {
    ...
    const noteParts = [
      SOURCE_TAG,
      r.cancelled ? (r.unitRaw ? 'Excel red / unmatched unit = cancelled' : 'No unit code = cancelled') : null,
      r.unitRaw && !r.unit ? `Unmatched unit: ${r.unitRaw}` : null,
    ...
    if (inserted <= 5 || inserted % 100 === 0 || inserted === EXPECTED) {
      console.log(
  ...
  const { rows: final } = await query(`SELECT count(*)::int AS n FROM reservations`);
  const { rows: byStatus } = await query(
    `SELECT status, count(*)::int AS n FROM reservations GROUP BY status ORDER BY status`
  );
  console.log(
    `[done] db_count=${final[0].n} inserted=${inserted} cancelled=${cancelled} null_unit=${nullUnit} blocked_nights=${blocked}`
  );
  console.log('[status]', byStatus);
  if (final[0].n !== EXPECTED) {
    throw new Error(`Final count ${final[0].n} !== ${EXPECTED}`);
  }
  await pool.end();
}
