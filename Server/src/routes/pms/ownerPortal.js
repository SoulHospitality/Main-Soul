...
router.get('/reports/owner-statement', requireRoles('owner', 'admin', 'finance', 'finance_manager', 'owners_relations', 'unit_acquisition_agent', 'unit_acquisition_manager'), async (req, res, next) => {
  try {
...
router.post('/owner/payout-requests/:id/review', requireRoles('admin', 'finance', 'finance_manager'), async (req, res, next) => {
  try {
    const status = String(req.body.status || '').toLowerCase();
