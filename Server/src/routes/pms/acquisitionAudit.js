...
router.get(
  '/acquisition-audit',
  requireRoles('admin', 'unit_acquisition_manager'),
  async (req, res, next) => {
    try {
      if (!isAdmin(req.user) && !isUnitAcquisitionManager(req.user)) {
      ...
      const teamQueryParams = admin ? [UNIT_ACQUISITION_ROLES] : [req.user.id, UNIT_ACQUISITION_ROLES];
      ...
      const { rows: events } = await query(
        ...
        [teamIds, ACQUISITION_AUDIT_ACTIONS, date]
      );
      ...
      const by_agent = [...byAgentMap.values()].sort((a, b) => b.count - a.count || a.full_name.localeCompare(b.full_name));
