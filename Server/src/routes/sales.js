...
router.get('/dashboard', async (req, res, next) => {
  try {
    const { rows: pending } = await query(
      `SELECT count(*)::int AS c FROM bookings WHERE status IN ('pending','held')`
    );
...
router.get('/bookings', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `SELECT * FROM bookings WHERE ${where} ORDER BY created_at DESC LIMIT $${params.length - 1} OFFSET $${params.length}`,
      params
    );
    res.json({ items: rows });
  } catch (err) {
  ...
  try {
    ...
    const { rows } = await query(
      `SELECT id, listing_title, listing_slug, guest_name, checkin, checkout, status, total_egp
       FROM bookings
       WHERE checkin < $2 AND checkout > $1 AND status IN ('confirmed','pending','held')
       ORDER BY checkin`,
      [from, to]
    );
