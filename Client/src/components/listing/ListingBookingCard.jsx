...
export default function ListingBookingCard({
  ...
  const inquiryLabel = t('listing.whatsappInquiry');
  ...
  return (
    <>
      <div className="md:sticky md:top-[116px] flex flex-col gap-4 rounded-3xl border border-soul-line bg-white p-6 shadow-[0_30px_70px_-35px_rgba(40,63,94,0.4)]">
        <div className="space-y-2 border-b border-soul-line pb-4">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-soul-muted">
            {inquiryOnly ? t('listing.inquiryOnly') : t('listing.reservation')}
          </p>
          ...
          {inquiryOnly ? (
            <p className="text-sm text-soul-muted">{t('listing.inquiryOnlyNote')}</p>
          ) : null}
        </div>
        <div className="flex flex-col gap-3">
          {inquiryOnly ? (
            whatsAppButton(
              'inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#25d366] px-6 py-3.5 text-sm font-semibold text-white shadow-[0_12px_28px_-10px_rgba(37,211,102,0.55)] transition-colors hover:bg-[#1ebe5a]'
            )
          ) : (
            <>
              <button
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white border-t border-soul-line px-4 py-3 flex flex-col gap-2 shadow-[0_-8px_24px_-12px_rgba(15,28,46,.18)]">
        {inquiryOnly ? (
          whatsAppButton(
            'inline-flex w-full items-center justify-center gap-2 rounded-[12px] bg-[#25d366] px-5 py-3 text-sm font-semibold text-white'
          )
        ) : (
          <>
